...
class TelegramService {
  ...
  async handleStart(msg) {
    ...
    await this.bot.sendMessage(chatId, "Please share your phone number to link your account", {
      reply_markup: {
  ...
  async handleContact(msg) {
    ...
    try {
      ...
      if (user) {
        ...
        await this.bot.sendMessage(chatId, "Phone number not registered. Please sign up first.");
      }
    } catch (error) {
...
// Core stores merchant and staff accounts. Storefront customers live in Commerce's
// database, so their Telegram links are handled by Commerce's TelegramService.
async findUser(phoneNumber, userType) {
  ...
  if (userType === 'merchant') {
    ...
    throw new Error('Telegram linking for customer accounts is handled by the Commerce API, not Core');
  } else {
  ...
  async sendVerificationOtp(user, chatId) {
    ...
    await this.bot.sendMessage(chatId, `Your verification code is: ${otp}\n\nPlease enter this code on our website.`);
  }
  ...
  async handleCustomerCancel(callbackQuery) {
    ...
    try {
      await axios.post(
        `${ecommerceApiUrl}/api/merchant/orders/${orderId}/process`,
        { action: "cancelled" }
      );
