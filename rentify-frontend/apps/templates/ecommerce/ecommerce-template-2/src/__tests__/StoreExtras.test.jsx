...
describe('CategoryCards', () => {
  test('links each arched card to its category and ignores unsafe image URLs', () => {
    const { container } = at(
      '/',
      <CategoryCards
        categories={[
    ...
    expect(screen.getByRole('link', { name: 'Explore Serums' }).getAttribute('href')).toBe('/products?category=c1');
    expect(screen.getByRole('link', { name: 'Explore Soaps' })).toBeTruthy();
