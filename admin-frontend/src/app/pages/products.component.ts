...
import { AdminService, Product } from '../services/admin-data.service';
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">Approve, edit, hide or remove marketplace listings</p>
      </div>
    </div>
    <div class="toolbar">
      ...
      <table class="tbl">
        ...
        <tbody>
          @for (p of rows(); track p.id) {
              ...
              <td>
                <div class="cell-flex">
                  ...
                  <div>
                    ...
                    @if (p.reported) { <span class="cell-sub" style="color:var(--red)">⚑ reported</span> }
                  </div>
                </div>
              </td>
              ...
              <td class="right">
                <div class="cell-actions">
                  @if (p.status === 'pending') {
                    ...
                    <button class="icon-btn" aria-label="Edit" data-tip="Edit" (click)="openEdit(p)">
                      <kc-icon name="edit" [size]="14"></kc-icon>
                    </button>
            ...
            <tr><td colspan="8"><div class="empty">No products match your filters.</div></td></tr>
          }
        </tbody>
      </table>
    </div>
    ...
    @if (edit(); as p) {
      <div class="modal-back">
        <div class="modal">
          <h3>Edit product</h3>
          <p>{{p.name}} — {{d.storeName(p.sellerId)}}</p>
          <div class="form-row">
            <label>Price (USD)</label>
            <input class="input" type="number" min="0" [value]="ePrice()" (input)="ePrice.set(+$any($event.target).value)"/>
          </div>
          <div class="form-row">
            <label>Stock</label>
            <input class="input" type="number" min="0" [value]="eStock()" (input)="eStock.set(+$any($event.target).value)"/>
          </div>
          <div class="form-row">
            <label>Status</label>
            <select class="input" [value]="eStatus()" (change)="eStatus.set($any($event.target).value)">
              <option value="active">Active</option>
              <option value="pending">Pending</option>
              <option value="hidden">Hidden</option>
              <option value="rejected">Rejected</option>
    ...
    @if (del(); as p) {
      <kc-confirm
        ...
        [message]="'“' + p.name + '” will be permanently removed from the marketplace.'"
        confirmLabel="Delete"
        (confirm)="d.deleteProduct(p.id); del.set(null)"
        (cancel)="del.set(null)"></kc-confirm>
    }
  } @else {
...
export class ProductsComponent {
  ...
  st = signal('all');
  selStore = signal('all');
  edit = signal<Product | null>(null);
  del = signal<Product | null>(null);
  ePrice = signal(0);
  eStock = signal(0);
  eStatus = signal<Product['status']>('active');
  money = money;
  initials = initials;

  rows = computed(() =>
    this.d.products().filter(
      (p) =>
  ...
  openEdit(p: Product) {
    ...
    this.eStatus.set(p.status);
    this.edit.set(p);
  }

  save(id: string) {
    this.d.updateProduct(id, { price: this.ePrice(), stock: this.eStock(), status: this.eStatus() });
    this.edit.set(null);
  }
}
