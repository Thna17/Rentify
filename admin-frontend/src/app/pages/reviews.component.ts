...
import { AdminService, Review } from '../services/admin-data.service';
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">Moderate buyer reviews and product feedback across the marketplace</p>
      </div>
    </div>
    <div class="card">
      <table class="tbl">
        ...
        <tbody>
          @for (r of d.reviews(); track r.id) {
            <tr>
              <td class="cell-main">{{r.product}}</td>
              <td>{{r.buyer}}</td>
              <td>
                <span class="stars">{{'★'.repeat(r.rating)}}</span>
                <span class="stars" style="opacity:.25">{{'★'.repeat(5 - r.rating)}}</span>
              </td>
              <td class="cell-sub">{{r.comment}}</td>
              <td class="cell-sub">{{dstr(r.date)}}</td>
              <td><kc-badge [value]="r.status"></kc-badge></td>
              <td class="right">
                <div class="cell-actions">
                  <button class="icon-btn"
                    [attr.data-tip]="r.status === 'visible' ? 'Hide review' : 'Show review'"
                    [attr.aria-label]="r.status === 'visible' ? 'Hide review' : 'Show review'"
                    (click)="d.setReviewStatus(r.id, r.status === 'visible' ? 'hidden' : 'visible')">
                    <kc-icon [name]="r.status === 'visible' ? 'eye-off' : 'eye'" [size]="14"></kc-icon>
                  </button>
    ...
    @if (del(); as r) {
      <kc-confirm
        title="Delete review?"
        [message]="'The review by ' + r.buyer + ' on ' + r.product + ' will be permanently removed.'"
        confirmLabel="Delete"
        (confirm)="d.deleteReview(r.id); del.set(null)"
