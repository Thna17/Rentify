...
@Component({
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Platform Notifications</h1>
        <p class="page-sub">Broadcast announcements sent to merchants, buyers and platform users</p>
      </div>
      <button class="btn btn-primary" (click)="open = true">Create notification</button>
    </div>
    ...
    @if (open) {
      <div class="modal-back">
        <div class="modal">
          ...
          <p>Sends an in-app announcement across the Rentify platform.</p>
          <div class="form-row">
            <label>Title</label>
            <input class="input" [value]="title()" (input)="title.set($any($event.target).value)"/>
          </div>
          <div class="form-row">
            <label>Message</label>
            <input class="input" [value]="body()" (input)="body.set($any($event.target).value)"/>
          </div>
          <div class="form-row">
