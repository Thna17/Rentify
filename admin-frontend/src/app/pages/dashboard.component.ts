  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">Unified Rentify Platform & Marketplace Administration</p>
      </div>
    </div>

    <!-- Platform & Storefront Operations Overview -->
    <div class="sect" style="margin-top:0">Platform & Storefronts</div>
    <div class="grid stats mb">
      <kc-stat label="Active Websites" icon="globe" [value]="d.websites().length" [hint]="'Vite storefronts running'"></kc-stat>
      <kc-stat label="Templates" icon="layout" [value]="d.templates().length" [hint]="'e-commerce & craft themes'"></kc-stat>
      <kc-stat label="Active Subscriptions" icon="package" [value]="d.subscriptions().length" [hint]="money(monthlyArr) + ' / month'"></kc-stat>
      <kc-stat label="Platform Users" icon="user" [value]="d.users().length" [hint]="'Merchants, staff, customers'"></kc-stat>
    </div>

    <!-- Marketplace Operations Overview -->
    <div class="sect">Marketplace Operations</div>
    <div class="grid stats mb">
    ...
    <div class="grid stats mb">
      <kc-stat label="Total Revenue" icon="wallet" [value]="money(d.revenue())" hint="commission + boost + subs"></kc-stat>
      ...
      <kc-stat label="Disputes & Complaints" icon="message" [value]="openComplaints"></kc-stat>
    </div>
    ...
    <div class="grid duo mb">
      <div class="card">
        <div class="card-head">
          ...
          <a class="link" routerLink="/orders">View all</a>
        </div>
        <table class="tbl">
          <thead>
            <tr><th>Order</th><th>Buyer</th><th class="right">Total</th><th>Payment</th><th>Status</th></tr>
          </thead>
          <tbody>
    ...
    <div class="grid g3">
          ...
          <tbody>
            @for (p of low; track p.id) {
              <tr>
                ...
                <td class="right">
                  <kc-badge [value]="p.stock === 0 ? 'hidden' : 'pending'"></kc-badge>
                  <span class="cell-sub"> {{p.stock}} left</span>
                </td>
              </tr>
            }
          </tbody>
      ...
      <div class="card">
        <div class="card-head">
          <h3 class="card-title">Recent seller registrations</h3>
          <a class="link" routerLink="/sellers">Review</a>
        </div>
        <div class="card-pad">
          @for (s of recentSellers; track s.id) {
            <div class="list-item">
              ...
              <div style="flex:1">
                <div class="cell-main">{{s.store}}</div>
                <div class="cell-sub">{{s.name}} · {{dstr(s.appliedAt)}}</div>
              </div>
...
export class DashboardComponent {
  ...
  openComplaints = this.d.complaints().filter((c) => c.status === 'open' || c.status === 'investigating').length;

  best = [...this.d.products()].sort((a, b) => b.sold - a.sold).slice(0, 5);
  low = this.d.products().filter((p) => p.stock <= 5).slice(0, 5);
  recentSellers = this.d.sellers().slice(-3).reverse();
