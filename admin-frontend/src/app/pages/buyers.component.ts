...
import { AdminService, Buyer } from '../services/admin-data.service';
...
@Component({
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Marketplace Buyers</h1>
        <p class="page-sub">View, search, suspend or deactivate buyer customer accounts</p>
      </div>
      <span class="muted">{{rows().length}} buyers</span>
    </div>
    <div class="toolbar">
        ...
        <tbody>
          @for (b of rows(); track b.id) {
            <tr>
              <td>
                <div class="cell-flex">
                  <span class="thumb">{{initials(b.name)}}</span>
                  <span class="cell-main">{{b.name}}</span>
                </div>
              </td>
              <td class="cell-sub">{{b.email}}</td>
              <td>{{dstr(b.registeredAt)}}</td>
              <td class="num">{{d.buyerStats(b.name).orders}}</td>
              <td class="num">{{money(d.buyerStats(b.name).spent)}}</td>
              <td><kc-badge [value]="b.status"></kc-badge></td>
              <td class="right">
                <div class="cell-actions">
                  <button class="icon-btn" aria-label="View details" data-tip="View details" (click)="sel.set(b)">
            ...
            <tr><td colspan="7"><div class="empty">No buyers match your filters.</div></td></tr>
          }
        </tbody>
      </table>
    </div>
    ...
    @if (sel(); as b) {
      ...
      <div class="drawer">
        <div class="drawer-head">
          <div><b>{{b.name}}</b> <kc-badge [value]="b.status"></kc-badge></div>
        ...
        <div class="drawer-body">
          ...
          @for (o of ordersOf(b); track o.id) {
            <div class="list-item">
              <div style="flex:1">
                <div class="cell-main">{{o.id}} · {{money(o.total)}}</div>
                <div class="cell-sub">{{o.items}} — {{o.seller}}</div>
              </div>
              <kc-badge [value]="o.status"></kc-badge>
            </div>
          ...
          <div class="cell-actions" style="justify-content:flex-start">
            @if (b.status === 'active') {
              ...
              <button class="btn btn-primary" (click)="ask(b, 'active')">Activate account</button>
            }
          </div>
        </div>
      </div>
    ...
    @if (confirm(); as c) {
      <kc-confirm
        [title]="(c.to === 'active' ? 'Activate ' : c.to === 'suspended' ? 'Suspend ' : 'Deactivate ') + c.b.name + '?'"
...
export class BuyersComponent {
  ...
  rows = computed(() =>
    this.d.buyers().filter(
      (b) =>
        (this.status() === 'all' || b.status === this.status()) &&
        (!this.q() ||
          b.name.toLowerCase().includes(this.q().toLowerCase()) ||
          b.email.toLowerCase().includes(this.q().toLowerCase())),
    ),
  );
  ...
  ask(b: Buyer, to: string) {
    this.confirm.set({ b, to: to as Buyer['status'] });
  }

  apply() {
    const c = this.confirm()!;
    this.d.setBuyerStatus(c.b.id, c.to);
