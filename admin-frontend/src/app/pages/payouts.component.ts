...
import { AdminService, Payout } from '../services/admin-data.service';
...
@Component({
  standalone: true,
  imports: [BadgeComponent, StatComponent, ConfirmComponent],
  template: `
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Seller Payouts</h1>
        <p class="page-sub">Approve, disburse and track settlement payouts to sellers</p>
      </div>
    </div>
      ...
      <table class="tbl">
        ...
        <tbody>
          @for (p of d.payouts(); track p.id) {
            <tr>
              <td class="cell-main">{{p.seller}}</td>
              <td class="num">{{money(p.amount)}}</td>
              <td class="cell-sub">{{dstr(p.requestedAt)}}</td>
    ...
    @if (confirm(); as p) {
      <kc-confirm
        ...
        [message]="money(p.amount) + ' will be marked as paid to ' + p.seller + '.'"
        confirmLabel="Complete"
        (confirm)="d.completePayout(p.id); confirm.set(null)"
        (cancel)="confirm.set(null)"></kc-confirm>
    }
  } @else {
