...
import { AdminService, Seller } from '../services/admin-data.service';
...
@Component({
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Marketplace Sellers</h1>
        <p class="page-sub">Review applications, verify, suspend and manage seller accounts</p>
      </div>
    </div>
    <div class="toolbar">
      <div class="tabs">
      ...
      <table class="tbl">
        ...
        <tbody>
          @for (s of rows(); track s.id) {
            <tr>
              <td>
                <div class="cell-flex">
                  ...
                  <span class="cell-main">{{s.store}}</span>
                </div>
              </td>
              <td>
              ...
              <td class="num">{{d.sellerStats(s.store).products}}</td>
              <td class="num">{{d.sellerStats(s.store).orders}}</td>
              <td class="num">{{money(d.sellerStats(s.store).revenue)}}</td>
              <td><kc-badge [value]="s.verification"></kc-badge></td>
              ...
              <td class="right">
                <div class="cell-actions">
                  @if (s.status === 'pending') {
                    ...
                    <button class="icon-btn" aria-label="View profile" data-tip="View profile" (click)="sel.set(s)">
            ...
            <tr><td colspan="8"><div class="empty">No sellers in this view.</div></td></tr>
          }
        </tbody>
      </table>
    </div>
      ...
      <div class="drawer">
        <div class="drawer-head">
          <div>
            ...
            <kc-badge [value]="s.verification"></kc-badge>
          </div>
          ...
          @for (p of productsOf(s); track p.id) {
            <div class="list-item">
              <div style="flex:1">
                ...
                <div class="cell-sub">{{money(p.price)}} · {{p.stock}} in stock · {{p.sold}} sold</div>
              </div>
          ...
          @for (o of ordersOf(s); track o.id) {
            <div class="list-item">
              <div style="flex:1">
                ...
                <div class="cell-sub">{{o.buyer}}</div>
              </div>
...
export class SellersComponent {
  ...
  rows = computed(() =>
    this.d.sellers().filter(
      (s) =>
        (this.tab() === 'all' || s.status === this.tab()) &&
        (!this.q() ||
          s.store.toLowerCase().includes(this.q().toLowerCase()) ||
  ...
  act(action: string, s: Seller) {
    ...
    if (action === 'suspend')
      this.confirm.set({
        fn: () => this.d.setSellerStatus(s.id, 'suspended'),
        msg: `Suspend ${s.store}? Their listings will be hidden from buyers.`,
      });
    if (action === 'reject')
      this.confirm.set({
        fn: () => this.d.rejectSeller(s.id),
        msg: `Reject ${s.store}? Their application will be closed.`,
      });
  }
}
