...
import { AdminService, Payment } from '../services/admin-data.service';
...
@Component({
  standalone: true,
  imports: [BadgeComponent, StatComponent, ConfirmComponent, MenuComponent],
  template: `
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Platform Payments</h1>
        <p class="page-sub">Order payments, KHQR, card, COD collections and refunds</p>
      </div>
    </div>
      ...
      <table class="tbl">
        ...
        <tbody>
          @for (p of d.payments(); track p.id) {
            <tr>
              <td class="cell-main">{{p.id}}</td>
              <td class="cell-sub">{{p.order}}</td>
              <td>{{p.buyer}}</td>
              <td class="cell-sub">{{p.method}}</td>
              <td class="num">{{money(p.amount)}}</td>
              <td><kc-badge [value]="p.status"></kc-badge></td>
              <td class="cell-sub">{{dstr(p.date)}}</td>
              <td class="right">
    ...
    @if (refund(); as p) {
      <kc-confirm
        ...
        [message]="money(p.amount) + ' will be refunded to ' + p.buyer + ' for ' + p.order + '.'"
        confirmLabel="Refund"
        (confirm)="d.refundPayment(p.id); refund.set(null)"
        (cancel)="refund.set(null)"></kc-confirm>
    }
  } @else {
...
export class PaymentsComponent {
  ...
  collected = computed(() =>
    this.d.payments().filter((p) => p.status === 'completed').reduce((s, p) => s + p.amount, 0),
  );
  pending = computed(() =>
    this.d.payments().filter((p) => p.status === 'pending').reduce((s, p) => s + p.amount, 0),
  );
  refunded = computed(() =>
    this.d.payments().filter((p) => p.status === 'refunded').reduce((s, p) => s + p.amount, 0),
  );
