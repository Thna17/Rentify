...
import { AdminService, Report } from '../services/admin-data.service';
...
@Component({
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Marketplace Reports</h1>
        <p class="page-sub">Buyer, seller and product reports — review, resolve or dismiss</p>
      </div>
    </div>
    <div class="toolbar">
      <div class="tabs">
      ...
      @for (r of rows(); track r.id) {
        <div class="card card-pad" style="cursor:pointer" (click)="sel.set(r)">
          <div style="display:flex;gap:10px;align-items:center;margin-bottom:6px">
            <kc-badge [value]="r.kind"></kc-badge>
            <b>{{r.target}}</b>
            <span class="spacer"></span>
            <kc-badge [value]="r.status"></kc-badge>
          </div>
          <p class="muted" style="margin:0 0 6px">“{{r.reason}}”</p>
          <div class="cell-sub">Reported by {{r.reporter}} · {{dstr(r.date)}}</div>
        </div>
      } @empty {
        <div class="card"><div class="empty">No reports in this view. 🎉</div></div>
      }
    </div>
    ...
    @if (sel(); as r) {
      ...
      <div class="drawer">
        <div class="drawer-head">
          <div><b>{{r.kind}} report</b> <kc-badge [value]="r.status"></kc-badge></div>
        ...
        <div class="drawer-body">
          ...
          @for (n of r.notes; track n.date) {
            <div class="list-item">
              <div>
                <div>{{n.text}}</div>
                <div class="cell-sub">{{dstr(n.date)}}</div>
              </div>
            </div>
          }
