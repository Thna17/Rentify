...
import { AdminService, Order } from '../services/admin-data.service';
...
@Component({
  ...
  @if (d.ready()) {
    <div class="page-head">
      <div>
        <h1 class="page-title">Marketplace Orders</h1>
        <p class="page-sub">Monitor orders, inspect details and cancel when necessary</p>
      </div>
    </div>
    <div class="toolbar">
      <div class="tabs">
        ...
        <tbody>
          @for (o of rows(); track o.id) {
            <tr>
              ...
              <td>{{o.buyer}}</td>
              <td>{{o.seller}}</td>
              <td class="cell-sub">{{dstr(o.placedAt)}}</td>
              <td class="num">{{money(o.total)}}</td>
              <td><kc-badge [value]="o.payment"></kc-badge></td>
              <td><kc-badge [value]="o.status"></kc-badge></td>
              <td class="right">
                <div class="cell-actions">
                  <button class="icon-btn" aria-label="View order" data-tip="View order" (click)="sel.set(o)">
            ...
            <tr><td colspan="8"><div class="empty">No orders in this view.</div></td></tr>
          }
        </tbody>
      </table>
    </div>
    ...
    @if (sel(); as o) {
      ...
      <div class="drawer">
        <div class="drawer-head">
          <div><b>{{o.id}}</b> <kc-badge [value]="o.status"></kc-badge></div>
    ...
    @if (cancel(); as o) {
      <kc-confirm
        title="Cancel order?"
        [message]="o.id + ' will be cancelled. The buyer and seller will see the updated status.'"
        confirmLabel="Cancel order"
        (confirm)="d.setOrderStatus(o.id, 'cancelled'); cancel.set(null); sel.set(null)"
        (cancel)="cancel.set(null)"></kc-confirm>
    }
  } @else {
